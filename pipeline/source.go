...
func Lines(ctx context.Context, path string) <-chan string {
	...
	go func() {
		...
		f, err := uio.Open(path)
		if err != nil {
			slog.Error("pipeline: cannot open source", slog.String("path", path), slog.String("error", err.Error()))
			return
		}
		defer f.Close()
		...
		if err := scanner.Err(); err != nil {
			slog.Error("pipeline: error reading source", slog.String("path", path), slog.String("error", err.Error()))
		}
	}()
	return out
}
