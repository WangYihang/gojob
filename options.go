...
// WithWorkers sets the number of concurrent workers (default 1). Non-positive
// values are ignored.
...
// WithTimeout bounds the runtime of a single attempt; zero (the default) means no
...
// WithRetry sets the maximum number of attempts per item (>= 1) and the backoff
